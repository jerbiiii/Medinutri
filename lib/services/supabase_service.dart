...
class SupabaseService {
  ...
  Future<String?> saveProfile(PatientProfile profile) async {
    try {
      ...
      debugPrint(
        '[SupabaseService] saveProfile for user_id: ${profile.userId}',
      );
      await _client.from('profiles').upsert(data, onConflict: 'user_id');
  ...
  Future<String?> saveNutritionPlan(NutritionPlan plan) async {
    try {
      debugPrint(
        '[SupabaseService] saveNutritionPlan for user: ${plan.userId}',
      );
  ...
  Future<String?> uploadProfilePhoto(String userId, File file) async {
    try {
      ...
      debugPrint(
        '[SupabaseService] Uploading to Storage: profiles/$storagePath',
      );
  ...
  /// ── FIX BUG #2 ──────────────────────────────────────────
  /// Returns (errorMessage, savedMedicationId).
  /// Uses `.select().single()` to retrieve the UUID generated by Supabase
  /// so the caller can immediately schedule notifications with the correct ID.
  Future<(String?, String?)> saveMedication(Medication medication) async {
    try {
      final data = medication.toMap();
      data.remove('id');

      if (medication.id != null) {
        // UPDATE — ID is already known
        await _client.from('medications').update(data).eq('id', medication.id!);
        return (null, medication.id);
      } else {
        // INSERT — retrieve the server-generated UUID
        final response = await _client
            .from('medications')
            .insert(data)
            .select('id')
            .single();
        final savedId = response['id']?.toString();
        debugPrint('[SupabaseService] saveMedication SUCCESS — id: $savedId');
        return (null, savedId);
      }
    } catch (e) {
